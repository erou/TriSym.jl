...
function tri_symmetric_search_coord2(b::Nemo.nmod_mat, E::Array{Nemo.fq_nmod, 1},
         d::Dict{Nemo.fq_nmod, Nemo.nmod_mat}, L::Array{Tuple{Vararg{Tuple{Nemo.fq_nmod,
         Int}}}, 1}, count::Int, bound::Int, G::Array{Tuple{Nemo.fq_nmod, Int},
         1} = Tuple{Nemo.fq_nmod, Int}[]) where N
    ...
    if r <= bound
        ...
        elseif count == 0
            ...
            for j in 1:len
                ...
                G2 = Base.copy(G) #not **deep** copy, be careful!
                push!(G2, (y, c))
                tri_symmetric_search_coord2(b2, B2, d, L, bound-1, G2)
            end
            ...
            for j in 1:length(E)
                ...
                for c in 1:p-1
                    ...
                    push!(G2, (x, c))
                    if r2 < r
                        tri_symmetric_search_coord2(b2, E2, d, L, count, bound-1, G2)
                    elseif r2 == r
                        tri_symmetric_search_coord2(b2, E2, d, L, count-1, bound-1, G2)
                    end
                end
            end
        end
...
function tri_symmetric_search_coord2(b::Nemo.nmod_mat, E::Array{fq_nmod, 1},
         d::Dict{Nemo.fq_nmod, Nemo.nmod_mat}, L::Array{Tuple{Vararg{Tuple{Nemo.fq_nmod,
         Int}}}, 1}, bound::Int, G::Array{Tuple{Nemo.fq_nmod, Int}, 1}) where N
...
function tri_symmetric_search2(B::BilinearMap{N}, d::Dict{Nemo.fq_nmod,
         Nemo.nmod_mat}, Lglob::Array{Tuple{Vararg{Tuple{Nemo.fq_nmod, Int}}},
         1}, counts::NTuple{N, Int}, bound::Int, j::Int = 1,
         Lloc::Array{Tuple{Nemo.fq_nmod, Int}, 1} = Tuple{Nemo.fq_nmod, Int}[]) where N
    ...
    elseif rank(B[j]) <= bound
        ...
        E = AffineFieldElements2(base_ring(B), j)
        tri_symmetric_search_coord2(B[j], E, d, M, count, bound)
        #res = tri_symmetric_search_coord(B[j], E, d, M, count, bound)
        #if res != nothing
        #    println("returned")
        #    return res
        ...
        for m in M
            ...
            Lloc2 = Base.copy(Lloc) #not **deep** copy, be careful!
            append!(Lloc2, m)
            tri_symmetric_search2(B2, d, Lglob, counts, bound-length(m), j+1, Lloc2)
            #res = tri_symmetric_search(B2, d, Lglob, counts, bound-length(m), j+1, Lloc2)
            #if res != nothing
            #    println("returned -")
            #    return res
...
function multiplication_search2(k::Nemo.FqNmodFiniteField, counts::NTuple{N,
                               Int}, bound::Int) where N
    B = multiplication_bilinear_map(k)
    d = make_conversion_dict(k)
    Lglob = Tuple{Vararg{Tuple{Nemo.fq_nmod, Int}}}[]
    
    tri_symmetric_search2(B, d, Lglob, counts, bound)
    return Lglob
end 
