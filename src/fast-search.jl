...
# =====================
#
# Algorithms searching for only one trisymmetric decomposition using the vector
# space structure of the algebra. The algorithms are essentially all the same,
# with minor differences in how the data is represented, resulting in different
# behaviour/speed in practice.
#
###############################################################################

export multiplication_fast_search
...
function tri_symmetric_fast_search(b::Nemo.nmod_mat, j::Int,
         B::BilinearMap{N}, E::Array{Nemo.fq_nmod, 1},
         d::Dict{Nemo.fq_nmod, Nemo.nmod_mat}, counts::Array{Int, 1},
         bound::Int, Lloc::Array{Tuple{Nemo.fq_nmod, Int}, 1},
         Lglob::Array{Tuple{Nemo.fq_nmod, Int}, 1},
         Lcoord::Array{Tuple{Nemo.fq_nmod, Int},
    ...
    if (r <= bound)#&& (length(Lglob) == 0)
        if r == 0
            # If r == 0, we must change the coordinate on which we are working,
            # so it is time to check if the work is finished or if we need to 
            # continue our search
            B2 = copy(B)
            add!(B2, Tuple(Lcoord), j, d)
            Lloc2 = Base.copy(Lloc)
            append!(Lloc2, Lcoord)
            if iszero(B2)
                # The work is done, we put it in Lglob
                append!(Lglob, Lloc2)
                Lglob = Base.copy(Lloc2)
            else
                # The work is not finished, we have to continue on the (j+1)-th
                # coordinate
                E2 = AffineFieldElements2(base_ring(B2), j+1)
                tri_symmetric_fast_search(B2[j+1], j+1, B2, E2, d, counts,
                                          bound, Lloc2, Lglob)
            end

        elseif count == 0
            # count == 0 so we do *not* have any "margin", we continue our search,
            # only with elements that decrease the rank
            b2 = similar(b)
            elems = Tuple{Nemo.fq_nmod, Int, Nemo.nmod_mat}[]
            
            # First we eliminate elements that are not able to make the rank
            # decrease; is it still worth it in that case?
            for x in E
                for c in 1:p-1
                    submul!(b2, b, d[x], c)
                    if rank(b2) < r
            ...
            for i in 1:length(elems)
                # We continue the search on the other elements of the search
                # tree, without changing B, j, Lloc, Lglob, because we stay on the same
                # coordinate
                if length(Lglob) != 0
                    break
                end
                y, c, b2 = elems[i]
                E2 = elems0[i+1:end]
                Lcoord2 = Base.copy(Lcoord) #not **deep** copy, be careful!
                push!(Lcoord2, (y, c))
                tri_symmetric_fast_search(b2, j, B, E2, d, counts, bound-1,
                                          Lloc, Lglob, Lcoord2)
            end

        else
            # count != 0 so we act a bit differently, but still we leave a lot
            # of variable unchanged: Lloc, Lglob, j, B, ...
            b2 = similar(b)
            for i in 1:length(E)
                if length(Lglob) != 0
                    break
                ...
                for c in 1:p-1
                    ...
                    if r2 < r
                        tri_symmetric_fast_search(b2, j, B, E2, d, counts, bound-1,
                                                  Lloc, Lglob, Lcoord2)
                    elseif r2 == r
                        counts2 = Base.copy(counts)
                        counts2[j] -= 1
                        tri_symmetric_fast_search(b2, j, B, E2, d, counts2, bound-1,
                                                  Lloc, Lglob, Lcoord2)
                    end
                end
            end
...
function prepare_search(k::Nemo.FqNmodFiniteField, counts::Array{Int, 1},
                        bound::Int) where N
    B = multiplication_bilinear_map(k)
    d = make_conversion_dict(k)
    Lglob = Tuple{Nemo.fq_nmod, Int}[]
    Lloc = Tuple{Nemo.fq_nmod, Int}[]
    j = 1
    b = B[j]
    E = AffineFieldElements2(k, j)
    
    return b, j, B, E, d, counts, bound, Lloc, Lglob
end

function multiplication_fast_search(k::Nemo.FqNmodFiniteField,
                                    counts::Array{Int, 1}, bound::Int) where N
    B = multiplication_bilinear_map(k)
    d = make_conversion_dict(k)
    Lglob = Tuple{Nemo.fq_nmod, Int}[]
    Lloc = Tuple{Nemo.fq_nmod, Int}[]
    ...
    tri_symmetric_fast_search(b, j, B, E, d, counts, bound, Lloc, Lglob)
    return Lglob
end 
